...
class PresentationExtensionTest : public XWalkExtensionsTestBase {
 public:
  ...
  void AttachDisplay(const gfx::Display& display) {
    PresentationDisplayManager::Get()->AddSecondaryDisplay(display);
  }

  // Simulate a secondray display is removed from system.
  void DetachDisplay(const gfx::Display& display) {
    PresentationDisplayManager::Get()->RemoveSecondaryDisplay(display);
  }
};

IN_PROC_BROWSER_TEST_F(PresentationExtensionTest, DisplayAvailable) {
  content::RunAllPendingInMessageLoop();
  GURL url = GetExtensionsTestURL(base::FilePath("presentation_api"),
